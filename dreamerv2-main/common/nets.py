...
class RSSM(common.Module):
  ...
  def imagine(self, action, state=None):
    ...
    action = swap(action)
    #print("Action:", tf.nest.flatten(action)[0].shape[0])
    prior = common.static_scan(self.img_step, action, state)
    #{print(v.shape) for k, v in prior.items()}
    prior = {k: swap(v) for k, v in prior.items()}
    return prior
  ...
  def get_feat(self, state):
    ...
    #print(tf.concat([stoch, state['deter']], -1).shape)
...
class ConvDecoder(common.Module):
  ...
  def __call__(self, features):
    ...
    x = self.get('flatten',tfkl.Flatten, None)(x)
    x = self.get('last', tfkl.Dense, 20 * 27, None)(x)
    mean = tf.reshape(x, tf.concat([tf.shape(features)[:-1], self._shape], 0))
    #print("Return(mode): ", tf.shape(features))
