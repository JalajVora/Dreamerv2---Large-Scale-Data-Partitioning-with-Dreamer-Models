...
class WorldModel(common.Module):
  ...
  def video_pred(self, data):
    data = self.preprocess(data)
    #print("Image Space: ", data['image'].shape)
    #print("Action Space: ", data['action'].shape)
    ...
    #print(data['action'][:6, 5:].shape)
    prior = self.rssm.imagine(data['action'][:6, :5], init)
    openl = self.heads['image'](self.rssm.get_feat(prior)).mode()
    #print([recon[:, :5] + 0.5, openl + 0.5])
    model = tf.concat([recon[:, :1] + 0.5, openl[:, :1] + 0.5], 1)
    #print(model.shape, truth.shape)
